...
//		_getsyscfg($this,url,func,"campusName","id");
	...
	$.delCookie = function(c_name){
	    ...
	    if(cval!=null) document.cookie= c_name + "="+cval+";max-age=0";
	}
	/**
	 * 记住用户名
	 */
	$.rememberMe = function(){
		...
		if( (ele.prop("tagName")=='INPUT'&&ele.is(':checked')) || (ele.prop("tagName")=='I'&&ele.hasClass('active'))){
			$.setCookie("account@"+document.domain,$.trim($("#login_account").val()),7);
		}else{
			$.delCookie("account@"+document.domain);
		}
	}
	/**
	 * 如果上次是记住用户名则勾选输入框
	...
	$.isRememberMe = function(){
		var mobile = $.getCookie("account@"+document.domain);
		var ele = $('#login_remeberMe');
		if(mobile){
