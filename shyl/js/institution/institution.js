    ...
    $(function () {
    	...
    	if (result == "emailErr") {
		    ...
		    $(".mypage_image").find("img").attr("src",document.getElementsByTagName('base')[0].href+$("#pictureUrl").val());
    ...
    function checkRegDate() {
    	...
    	//和当前时间比较
    	if (new Date(RegDate).getTime() > new Date().getTime()) {
    		$("#RegDate").tips({
    			side : 1,
    			msg : '注册时间必须小于当前时间',
    			bg : '#AE81FF',
	...
	$(function() {
		 ...
		 $("#file_upload").fileupload({
			 ...
			 autoUpload: true,
			 maxNumberOfFiles : 1,
	         url: document.getElementsByTagName('base')[0].href+'register/Upload.act',
	...
	function showImgDelayNew(index){
		...
		imgObj.onerror=null;
		imgObj.src=document.getElementsByTagName('base')[0].href+'shyl/images/mylogo.jpg';  
	}  
	// 不存在的图片处理2
	...
	$(function() {
		 ...
		 $("#file_uploadC").fileupload({
			dataType :'json',
			autoUpload: true,
	        url: locat+'institutionCtrl/Upload.act',
	        sequentialUploads: true
		 }).bind('fileuploadchange', function (e, data) {
	    }).bind('fileuploaddone', function (e, data) {
