				...
				success : function(data) {
					...
					for (var i = 0; i < List.length; i++) {
						...
						if(List[i].end_time!=null){
							end1 = List[i].end_time.substring(0, 7);
						}
						
								html += '<li style="line-height:40px;padding-left:8px">',
								html += '<div style="width:850px;margin-left:8px;margin-bottom:-30px">'
										+ start1
										+ '～'
										+ end1
										+ ':'
										...
										+ '/'
										+ '海外学习经历:'
										+ List[i].overseas_study1 + '</div>',
								html += '<span style="float:right;margin-top：30px">',
								html += '<a href="#" class="modify" data-toggle="modal" data-target="#eEdu" onclick="eduUpdate('
										+ List[i].info_id
										+ ')">'
										+ '修改'
										+ '</a>',
								html += '<a href="#" class="delete" data-toggle="modal" data-target="#deledu" onclick="eduDelete('
										+ List[i].info_id
										+ ')">'
										+ '删除'
										+ '</a>', html += '</span>',
					...
					for (var i = 0; i < lanlist.length; i++) {
								...
								lanhtml += '<span>' + '语言:'
										+ lanlist[i].languages + '/' + '听力能力:'
										+ lanlist[i].listen_say_skill + '/'
										+ '读写能力:' + lanlist[i].read_write_skill
										+ '</span>',
								lanhtml += '<span style="float:right;margin-left:250px;">',
								lanhtml += '<a href="#" class="modify" data-toggle="modal" data-target="#LansEdit" onclick="lanUpdate('
										+ lanlist[i].info_id
										+ ')">'
										+ '修改'
										+ '</a>',
								lanhtml += '<a href="#" class="delete" data-toggle="modal" data-target="#dellan" onclick="lanDelete('
										+ lanlist[i].info_id
										+ ')">'
										+ '删除'
										+ '</a>', lanhtml += '</span>',
					...
					for (var i = 0; i < worklist.length; i++) {
								...
								workhtml += '<li style="line-height:30px;float;right;">',
								workhtml += '<span style="width:70%">'+ '<span style="text-align:right;display:block;width:8%;float:left;padding-left:15px">'+ '企业名称:' + '</span>'+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+ worklist[i].enterprise_name+ '</span>',
								workhtml += '<span style="float:right;position:relative;">',
								workhtml += '<a href="#" class="modify" data-toggle="modal" data-target="#WorkEdit" onclick="workrefresh('
										+ worklist[i].info_id
										+ ')">'
										+ '修改'
										+ '</a>',
								workhtml += '<a href="#" class="delete" data-toggle="modal" data-target="#delwork" onclick="workDelete('
										+ worklist[i].info_id
										+ ')">'
										+ '删除'
										+ '</a>',
								...
								workhtml += '<br>'
								workhtml += '<span>'
										+ '<span style="text-align:right;display:block;width:8%;float:left;padding-left:15px">'
										+ '企业性质:' + '</span>'
										+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
										+ worklist[i].enterprise_nature
										+ '</span>', workhtml += '<br>'
								workhtml += '<span>'
										+ '<span style="text-align:right;display:block;width:8%;float:left;padding-left:15px">'
										+ '行业类别:' + '</span>'
										+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
										+ worklist[i].industry_type + '</span>',
								workhtml += '<br>'
								workhtml += '<span style="width:70%">'
										+ '<span style="text-align:right;display:block;width:8%;float:left;padding-left:15px">'
										+ '部门:' + '</span>'
										+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
										+ worklist[i].department + '</span>',
								workhtml += '<br>'
								workhtml += '<span>'
										+ '<span style="text-align:right;display:block;width:8%;float:left;padding-left:15px">'
										+ '时间:' + '</span>'
										+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
										+ worklist[i].start_time.substring(0, 7) + '～'
										+ end1 + '</span>',
								workhtml += '<br>'
								workhtml += '<span>'
										+ '<span style="text-align:right;display:block;width:8%;float:left;padding-left:15px">'
										+ '职位:' + '</span>'
										+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
										+ worklist[i].positions + '</span>',
								workhtml += '<br>'
								workhtml += '<span>'
										+ '<span style="text-align:right;display:block;width:8%;float:left;padding-left:15px">'
										+ '下属人数:' + '</span>'
										+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
										+ worklist[i].subordinates + '</span>',
								workhtml += '<br>'
								workhtml +='<span style="text-align:right;display:block;width:8%;float:left;padding-left:15px">'
										+ '工作描述:' + '</span>'
										+'<div style="width:700px;margin-left:105px;">'
										+ worklist[i].job_description
										+ '</div>',
								workhtml += '</li>',
					...
					for (var i = 0; i < prolist.length; i++) {
						prohtml += '<li style="line-height:30px;float;right;padding-left:15px">',
								prohtml += '<span>'
										...
										+ '</span>'
										+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
										+ prolist[i].project_name + '</span>',
										prohtml += '<span style="float:right;position:relative;">',
										prohtml += '<a href="#" class="modify" data-toggle="modal" data-target="#Programedit" onclick="prorefresh('
												+ prolist[i].info_id
												+ ')">'
												+ '修改'
												+ '</a>',
										prohtml += '<a href="#" class="delete" data-toggle="modal" data-target="#delpro" onclick="deletepro('
												+ prolist[i].info_id
												+ ')">'
												+ '删除'
												+ '</a>', prohtml += '</span>',
										prohtml += '<br>'
						prohtml += '<span>' + '<span style="padding-left:32px">'
								+ '时间:' + '</span>'
								+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
								+ prolist[i].start_time.substring(0,7) + '～'
								+ prolist[i].end_time.substring(0,7) + '</span>',
								prohtml += '<br>'
						prohtml +='<span>'
								+ '开发工具:' + '</span>'
								+'<div style="width:700px;margin-left:90px;margin-top:-30px;margin-bottom:-30px">'
								+ prolist[i].development_tool+'</div>',
								prohtml += '<br>'
						prohtml +='<span>'
								+ '涉及技术:' + '</span>'
								+'<div style="width:700px;margin-left:90px;margin-top:-30px;margin-bottom:-30px">'
								+ prolist[i].technology + '</div>',
						prohtml += '<br>'
						prohtml +='<span>'
								+ '项目描述:' + '</span>'
								+'<div style="width:700px;margin-left:90px;margin-top:-30px;margin-bottom:-30px">'
								+ prolist[i].project_description + '</div>',
								prohtml += '<br>'
								prohtml +='<span>' + '职位描述:'
										+ '</span>'
										+'<div style="width:700px;margin-left:90px;margin-top:-30px;">'
										+ prolist[i].job_description
										+ '</div>',
								prohtml += '</li>',
					...
					for (var i = 0; i < tralist.length; i++) {
						trahtml += '<li style="line-height:30px;float;right">',
								trahtml +='<span  style="padding-left:15px">' + '培训机构:'
										+ '</span>'
										+ '<div style="width:700px;margin-left:100px;margin-top:-30px;margin-bottom:-30px">'
										+ tralist[i].training_institutions
										+ '</div>', 
										trahtml += '<span style="float:right;position:relative;">',
										trahtml += '<a href="#" class="modify" data-toggle="modal" data-target="#traedit" onclick="trarefresh('
												+ tralist[i].info_id	
												+ ')">'
												+ '修改'
												+ '</a>',
										trahtml += '<a href="#" class="delete" data-toggle="modal" data-target="#deltra" onclick="deletetra('
												+ tralist[i].info_id
												+ ')">'
												+ '删除'
												+ '</a>', trahtml += '</span>',
										trahtml += '<br>'
						trahtml += '<span>' + '<span  style="padding-left:40px">'
								+ '时间:' + '</span>'
								+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
								+ tralist[i].start_time.substring(0,7) + '～'
								+ tralist[i].end_time.substring(0,7) + '</span>',
								trahtml += '<br>'
						trahtml += '<span>' + '<span  style="padding-left:40px">'
								+ '证书:' + '</span>'
								+ '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
								+ tralist[i].certificate + '</span>',
								trahtml += '<br>'
								trahtml +='<span  style="padding-left:13px">' + '培训课程:'
										+ '</span>'
										+'<div style="width:700px;margin-left:100px;margin-top:-30px;">'
										+ tralist[i].course + '</div>',
								trahtml += '</li>',
					...
					for (var i = 0; i < itList.length; i++) {
								...
								ithtml += '<div class="form-control-static" style="width:850px;padding-left:15px">'
										+ '名称:'
										+ itList[i].skill_name
										+ '/'
										+ '使用时间:'
										...
										+ '/'
										+ '掌握程度:'
										+ itList[i].skill_level
										+ '</div>',
								ithtml += '<span style="float:right;margin-top:-30px">',
								ithtml += '<a href="#" class="modify" data-toggle="modal" data-target="#Itedit" onclick="itrefresh('
										+ itList[i].info_id
										+ ')">'
										+ '修改'
										+ '</a>',
								ithtml += '<a href="#" class="delete" data-toggle="modal" data-target="#delit" onclick="itDelete('
										+ itList[i].info_id
										+ ')">'
										+ '删除'
										+ '</a>', ithtml += '</span>',
					...
					for (var i = 0; i < honList.length; i++) {
								...
								honhtml += '<div class="form-control-static" style="width:850px;padding-left:15px">'
										+ '证书名称:'
										+ honList[i].certificate_name
										+ '/'
										+ '证书获得时间:'
										+ honList[i].get_time.substring(0,7) 
										+ '</div>',
								honhtml += '<span style="float:right;margin-top:-30px">',
								honhtml += '<a href="#" class="modify" data-toggle="modal" data-target="#honedit" onclick="honrefresh('
										+ honList[i].info_id
										+ ')">'
										+ '修改'
										+ '</a>',
								honhtml += '<a href="#" class="delete" data-toggle="modal" data-target="#delhon" onclick="honDelete('
										+ honList[i].info_id
										+ ')">'
										+ '删除'
										+ '</a>', honhtml += '</span>',
					...
					$("#prolist").html(prohtml);
					$("#tralist").html(trahtml);
					$("#itlist").html(ithtml);
					$("#honlist").html(honhtml);
				}
			});
}
function lanSave() {
	...
	if (valitework()) {
		$.ajax({
			...
			success : function(data) {
				showinfo();
				$('#Work').modal('hide');
				$("input[name='reswork']").trigger("click");
				// location.reload();
				return false;
			}
		});
	...
	if (vilatepro()) {
		$.ajax({
			...
			success : function(data) {
				showinfo();
				$('#Program').modal('hide');
				$("input[name='respro']").trigger("click");
				return false;
			}
		});
	}
...
$("#TraButton").click(function() {
	...
	if (valatetra()) {
		$.ajax({
			...
			success : function(data) {
				showinfo();
				$("input[name='restra']").trigger("click");
				$('#Tra').modal('hide');
			}
		});
	}
...
$("#ItButton").click(function() {
	...
	if (valateit()) {
		$.ajax({
			...
			success : function(data) {
				showinfo();
				$("input[name='resit']").trigger("click");
				$('#It').modal('hide');
			}
		});
	}
...
$("#HonButton").click(function() {
	...
	if (valatehon()) {
		$.ajax({
			...
			cache : false,
			success : function(data) {
				$("input[name='reshon']").trigger("click");
				$('#hon').modal('hide');
				showinfo();
			}
		});
...
/*		// 时间和当前日期比较
		if (BeginDate.length > 0 && currentdate.length > 0) {
...
function Eduupdate() {
	...
	var HasOverseas1 = $("input:radio[name='HasOverseasupdate']:checked").val();
	$.ajax({
		type : "POST",
...
function eduinfo() {
	var CV_ID = $("#cvid").val();
	window.location.href = locat + '/myresume/edushow.act?CV_ID=' + CV_ID;
}
function basicinfo() {
	$("#edu").removeClass("on");
	$("#basic").addClass("on");
...
function eduUpdate(info_id) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			$("#info_id").val(info_id);
			if (overseas_study == '0') {
				$("input[type=radio][name=HasOverseasupdate][value=0]").attr(
						"checked", 'checked')
			} else {
				$("input[type=radio][name=HasOverseasupdate][value=1]").attr(
						"checked", 'checked')
			}
		}
	});
...
$("#EduButtonedu").click(function() {
	...
	var HasOverseas = $("input:radio[name='HasOverseas']:checked").val();
	if (valiteedu()) {
		$.ajax({
			type : "POST",
			url : locat + '/edu/edusave.act?tm=' + new Date().getTime(),
			data : {
				CV_ID : CV_ID,
				school_name : school,
				start_time : start,
			...
			success : function(data) {
				...
				showinfo();
				$('#sEdu').modal('hide');
				$("input[name='resedu']").trigger("click");
			}
		});
	}
	function valiteedu() {
