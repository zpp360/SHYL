...
<html lang="en">
<head>
<title>人员竞标详情编辑画面_济南市服务外包公共服务平台</title>
<meta charset="UTF-8" />
...
<body id="top" style="background-color: #f7f7f7;">
	...
	<div class="register-box p0">
		...
		<input id="personnel_id" type="hidden" value="${pd.personnel_id}">
			...
			<form>
				<ul >
					<li>
						...
						<input id="contact"
							...
							value="${pd.contact}"
							maxlength="50"
							class="inputtxt2" onblur="trim(this);">
					</li>
				...
				<a  class="sback"  
					href="<%=basePath%>requirementdetail/personnelBidListInfo.act">返回</a>
...
<script type="text/javascript" src="<%=basePath%>shyl/js/jquery.tips.js"></script>
	...
	//文件上传
	 $("#file_upload").fileupload({
         ...
         url: '<%=basePath%>PersonnelBidDetail/Upload.act',
			...
			if ("no" == data.result) {
				//alert("请上传格式为.doc/.docx/.xls/.xlsx/.ppt/.pptx/.txt/.jpg格式的文档文件。");
...
<script type="text/javascript"
	...
	function ibtnModify_Click() {
		...
		$.ajax({
			type : "POST",
			url : '<%=basePath%>PersonnelBidDetail/update.act',
			data : {
