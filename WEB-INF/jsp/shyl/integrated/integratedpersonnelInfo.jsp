...
<head>
<meta charset="utf-8">
<title>人员需求_济南市服务外包公共服务平台</title>
<base href="<%=basePath%>">
...
<script src="<%=basePath%>shyl/js/integrated/integratedpersonnelInfo.js" type="text/javascript"></script>
...
<div class="register-box clearfix">
	...
	<div class="bidetailed mt40">
    	<p class="biddetailed-tit" onclick="test();">${pd.personnel_title}</p>
       <p class="style2-big fl"><span class="style3">发布企业：</span><span class="n-tit-big"><a href="<%=basePath%>company/info.act?companyId=${pd.insert_user}" style="color:#016ab9;">${pd.publish_company_name}</a></span></p>
        <p class="style2 fl"><span class="style3">发布日期：</span><span>${pd.update_timestamp}</span></p>
    </div>
    <div class="bidetailed mt40">
    	<p class="biddetailed-tit">业务明细</p>
        <p class="style2-textarea fl"><span class="style5">IT技能：</span><span class="n-tit-textarea">${pd.it}</span><div class="clear"></div></p>
        <p class="style2-textarea fl"><span class="style5">行业知识：</span><span class="n-tit-textarea">${pd.hy}</span><div class="clear"></div></p>
        <p class="style2 fl"><span class="style5">语言要求：</span><span>${pd.language_level}</span></p>
        <p class="style2 fl"><span class="style5">需求人数：</span><span>${pd.requirement_personel_count}</span></p>
        <p class="style2 fl"><span class="style6">读写能力：</span><span>${pd.literacy}</span></p>
        <p class="style2 fl"><span class="style5">听说能力：</span><span>${pd.speaking_skills}</span></p>
        <p class="style2 fl"><span class="style5">需求有效期：</span><span>${pd.validity_period}</span></p>
        <p class="style2 fl"><span class="style6">项目周期(月)：</span><span>${pd.project_month}</span></p>
        <p class="style2 fl"><span class="style5">承包方省份：</span><span>${pd.contractor_province}</span></p>
        <p class="style2 fl"><span class="style5">承包方市：</span><span>${pd.contractor_city}</span></p>
        
        <c:if test="${ pd.sex =='0'}">
        	<p class="style2 fl"><span class="style6">联系人：</span><span>${pd.contact}&nbsp;先生&nbsp;</span></p>
        </c:if>
        <c:if test="${ pd.sex =='1'}">
        	<p class="style2 fl"><span class="style6">联系人：</span><span>${pd.contact}&nbsp;女士&nbsp;</span></p>
        </c:if>
        
	   <p class="style2 fl"><span class="style5">联系电话：</span><span>${pd.contact_tel}
        </span></p>
        <p class="style2 fl"><span class="style5">E-Mail：</span><span class ="n-tit-small-200" >${pd.email}</span></p>
        <c:if test="${pd.path_flag == '1'}">
        <p class="style2 fl"><span class="style6">附件：</span><span>无</span></p>
        </c:if>
        <c:if test="${pd.path_flag == '0'}">
        <p class="style2 fl"><span class="style6">附件：</span><a href="javascript:void(0);" class="more" onclick="dounfile()"
					id="creatPDF" style="position:relative;">&nbsp;&nbsp;下载 &nbsp;&nbsp;</a></p>
        </c:if>
		...
		<c:when test="${not empty page.flag}">
		 <div class="anniu jbanniu">
      	<button class="ljjb" data-toggle='modal' data-target='' class='modify' onclick="logint('${pd.personnel_id}', '${pd.insert_user}');" id="ljzb">立即竞标</button>
        </div>
		</c:when>
		<c:otherwise>
		</c:otherwise>
