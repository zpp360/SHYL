...
<br>
					...
					<td><select class="chzn-select" name="film_name" id="film_name"
						data-placeholder="请选择放映场所"  style="height: 20px; width: 315px;">
										...
										<c:forEach items="${filmlist}" var="fl">
											<option value="${fl.id }" <c:if test="${fl.id == pd.sc_address}">selected</c:if>>${fl.value }</option>
										</c:forEach>
					</select></td>
				</tr>
				<tr>
	...
	<script type="text/javascript">
		...
		ue.URL = "http://localhost:8080/shyl";
		var URL = window.UEDITOR_HOME_URL || getUEBasePath();
	    var prefixUrl = window.document.location.origin; 
		//ueditor有标签文本
