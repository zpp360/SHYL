	...
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="<%=basePath%>app/suggestion_list.do" method="post" name="Form" id="Form">
...
<!-- 						</table> -->
						<table id="table_report"
							...
							<thead>
								<tr>
									...
									<th class="center" style="width:12%;">用户名</th>
									<th class="center" style="width:15%;">用户单位</th>
											...
											<c:forEach items="${pdList}" var="list" varStatus="vs">
												<tr>
													<td class='center' ><label><input
															type='checkbox' name='ids' value="${list.suggestion_id}" /><span
															class="lbl"></span></label></td>
													<td class='center' >${vs.index+1}</td>
													<td class="center">${list.user_name}</td>
													<td class="center">${list.unit_name }</td>
													<td class="tds">${list.suggestion_content }</td>
													<td class="center">
														<fmt:formatDate value="${list.create_time}" pattern="yyyy-MM-dd HH:mm:ss"/> 
													</td>
													<td style="width: 50px;" class="center">
														<a class="btn btn-small btn-danger" onclick="del('${list.suggestion_id}');" title="删除"><i class="icon-trash"></i></a>
														<a class="btn btn-small btn-info" onclick="view('${list.suggestion_id}');" title="查看">查看</a>
													</td>
												</tr>
											</c:forEach>
									</c:when>
	...
	<script type="text/javascript">
		...
		//删除
		function del(suggestion_id,msg){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>app/del_suggestion.do?suggestion_id="+suggestion_id+"&tm="+new Date().getTime();
		...
		//查看详情
		function view(suggestion_id){
			top.jzts();
			var diag = new top.Dialog();
			diag.Drag=true;
			diag.Title ="新增菜单";
			diag.URL = dialogPath+'app/view_suggestion.do?suggestion_id='+suggestion_id;
			diag.Width = 400;
			diag.Height = 200;
			 ...
			 bootbox.confirm(msg, function(result) {
				if(result) {
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>app/batchDelSuggestion.do?tm='
																+ new Date()
																		.getTime(),
														 data : {
															IDS : str
