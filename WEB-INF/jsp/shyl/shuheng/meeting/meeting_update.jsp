...
<script type="text/javascript" src="https://api.map.baidu.com/api?v=3.0&ak=CIj675KvsEVU85xdW0OeLSFi5zgUc2EZ&s=1"></script>
<!-- 添加按钮 -->
...
<body>
	<br>
	<form action="<%=basePath%>meeting/update.do" name="Form" id="Form" method="post">
			<input type="hidden" name="meeting_id" value="${pd.meeting_id}"/>
			<table class="table">
				<tr>
					...
					<td>
						<select id="sign_up_id" class="chzn-select" name="sign_up_id"  data-placeholder="请选择活动"  style="height: 20px; width: 315px;">
								...
								<c:forEach items="${signUpList}" var="list">
									<option value="${list.sign_up_id}" <c:if test="${pd.meeting_sign_up_id == list.sign_up_id }">selected</c:if>>${list.sign_up_title}</option>
								</c:forEach>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td>
					    <input type="text" name="meeting_title" id="meeting_title" value="${pd.meeting_title}" maxlength="200" style="height: 20px; width: 300px;" placeholder="请输入会议名称" title="会议名称" onblur="trim(this);" />
					</td>
				</tr>
				<tr>
					<td style="width: 100px; text-align: left; padding-top: 13px;">
				...
				<tr>
					...
					<td>
					    <input type="hidden" name="meeting_regist_place" id="meeting_regist_place" value="${pd.meeting_regist_place}" maxlength="200" style="height: 20px; width: 300px;" placeholder="${pd.meeting_regist_place}" title="报到地点" onblur="trim(this);" />
					    <input type="text" name="meeting_regist_place_display" id="meeting_regist_place_display" value="${pd.meeting_regist_place}" maxlength="200" style="height: 20px; width: 300px;" placeholder="${pd.meeting_regist_place}" title="报到地点" onblur="trim(this);" />
				...
				<tr>
					<td style="width: 100px; text-align: left; padding-top: 13px;">
						<B>报到联系人</B></td>
					<td>
						<textarea name="meeting_linkman_txt" id="meeting_linkman_txt" maxlength="200" style="height: 80px; width: 300px;" placeholder="请输入报到联系人及联系方式" title="报到联系人" onblur="trim(this);">${pd.meeting_linkman_txt}</textarea>
					</td>
				</tr>
				<tr>
					<td style="width: 100px; text-align: left; padding-top: 13px;">
					...
					<td>
					    <input type="hidden" name="meeting_place" id="meeting_place" value="${pd.meeting_place}" maxlength="200" style="height: 20px; width: 300px;" placeholder="${pd.meeting_place}" title="会议地点" onblur="trim(this);" />
					    <input type="text" name="meeting_place_display" id="meeting_place_display" value="${pd.meeting_place}" maxlength="200" style="height: 20px; width: 300px;" placeholder="${pd.meeting_place}" title="会议地点" onblur="trim(this);" />
				...
				<tr>
					...
					<td>
						<div class="upload_img">
							<input id="meeting_img" name="meeting_img" type="hidden" value="${pd.meeting_img }"/>
							...
							<c:if test="${!empty pd.meeting_img}">
						        <img id="weixin_show" name="weixin_show" style="width: auto; height: 150px;max-width:200px;" onerror="showImgDelay(this,'<%=basePath%>shyl/images/temp.png');" src="${img_host}${pd.meeting_img}">
						    </c:if>
						...
						<div class="cut_img">
							...
							<input type="hidden" id="cutImage" name="cutImage" value="${pd.meeting_img}" placeholder="裁切图片"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<div align="center" style="margin-top: 10px;">
			...
			<a class="btn  btn-success" onclick="update();"><i
				class="icon-save"></i>更新</a> <a class="btn  btn-light"
    ...
    var registPlace = new BMap.Autocomplete(    //建立一个自动完成的对象
        {"input" : "meeting_regist_place_display"
        ,"location" : registMap
    });
    ...
    function onHighLight(ac,map,idStr){
		...
		$(document).ready(function(){
			$("#meeting_regist_place_display").focus(function(){
				$("#l-map").show();
			});
			$("#meeting_regist_place_display").blur(function(){
				$("#l-map").hide();
			});
			$("#meeting_place_display").focus(function(){
				$("#meeting_place_map").show();
			});
			$("#meeting_place_display").blur(function(){
