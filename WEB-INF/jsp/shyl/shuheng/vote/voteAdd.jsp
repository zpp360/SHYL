...
<body>
	<br>
	<form action="<%=basePath%>vote/saveVote.do" name="Form" id="Form" method="post">
		...
		<div id="zhongxin">
			<table class="table">
				<tr>
					...
					<td>
						<input type="text" name="vote_name" id="vote_name" <c:if test="${lgbj_level!=2}">disabled="disabled"</c:if> value="${votePd.vote_name }" maxlength="50" style="height: 20px; width: 300px;" placeholder="请输入规则名称" title="规则名称" onblur="trim(this);"/>
					</td>
				</tr>
				<tr>
					<td style="width: 100px; text-align: left; padding-top: 13px;">
				...
				<tr>
					...
					<td>
						每人<input type="number" name="vote_day" id="vote_day" <c:if test="${lgbj_level!=2}">disabled="disabled"</c:if> <c:if test="${votePd.vote_day==null }">value="1"</c:if> <c:if test="${votePd.vote_day!=null }">value="${votePd.vote_day}"</c:if> maxlength="3" min="0" style="height: 20px; width: 90px;" placeholder="请输入天数" title="天数" onblur="trim(this);"/>天&nbsp;&nbsp;&nbsp;&nbsp;   
						<input type="number" name="vote_ticket" id="vote_ticket" <c:if test="${lgbj_level!=2}">disabled="disabled"</c:if> <c:if test="${votePd.vote_ticket==null }">value="1"</c:if> <c:if test="${votePd.vote_ticket!=null }">value="${votePd.vote_ticket}"</c:if> maxlength="3" min="1" style="height: 20px; width: 90px;" placeholder="请输入票数" title="票数" onblur="trim(this);"/>票
					</td>
				</tr>
				<tr>
					<td style="width: 100px; text-align: left; padding-top: 13px;">
					...
					<td>
						<span>
							<input name="vote_upload_end_time" id="vote_upload_end_time" class="Wdate" value="" type="text"  placeholder="请选择上传团队资料截止时间" onclick="WdatePicker({maxDate:'#F{$dp.$D(\'vote_begin_time\',{d:-0});}',dateFmt:'yyyy-MM-dd HH:mm',isShowClear:true});"/>
						</span>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<input type="checkbox" name="vote_push" id="vote_push"  value="1" placeholder="选择是否是超级管理员" title="是否是超级管理员" style="opacity:1;position: relative;"/>
					</td>
				</tr>
				</c:if>
				<tr>
			...
			<c:if test="${lgbj_level==2}">
				<textarea id="vote_rule_txt" name="vote_rule_txt" maxlength="10000" title="活动通知内容" style="width:600px; height:300px;"></textarea>
			</c:if>
		</div>
