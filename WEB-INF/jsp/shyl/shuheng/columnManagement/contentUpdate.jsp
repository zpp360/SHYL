		...
		<div id="zhongxin">
			<table class="table">
				...
				<tr>
					...
					<td>
						<input type="text" name="news_source" id="news_source" value="${pd.news_source}" maxlength="50" style="height: 20px; width: 300px;" placeholder="默认为当前单位名称" title="新闻来源" onblur="trim(this);"/>
					</td>
				</tr>
				<tr>
					<td style="width: 100px; text-align: left; padding-top: 13px;">
...
<%-- 						<label style="float:left;padding-left: 32px;"><input name="del_flag" id="del_flag" <c:if test="${pd.del_flg == '0'}">checked="checked</c:if> value="0" type="radio"><span class="lbl">发布</span></label> --%>
<%-- 						<label style="float:left;padding-left: 5px;"><input name="del_flag" id="del_flag" <c:if test="${pd.del_flg == '2'}">checked="checked</c:if> value="2" type="radio"><span class="lbl">草稿</span></label> --%>
<!-- 					</td> -->
<!-- 				</tr> -->
				<tr>
...
// 		    		return false;
// 		    	}
				$("#Form").attr("action","<%=basePath%>columnManagement/updateContent.do?del_flag="+del_flag);
		    	$("#Form").submit();
		    	$("#zhongxin").hide();
		    	$("#zhongxin2").show();
		...
		function phoneView(phone){
			...
			$("#Form").append("<input type='hidden' name='back' value='update'/>");
			$("#Form").submit();
		}
	</script>
</body>
