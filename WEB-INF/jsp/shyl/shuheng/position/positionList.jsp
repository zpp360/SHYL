...
<body>
						...
						<table id="table_report"
							...
							<thead>
								<tr>
									...
									<th class="center" style="width:11%;">操作</th>
								</tr>
							</thead>
							<tbody>
								<!-- 开始循环 -->
										...
										<c:forEach items="${varList}" var="ps" varStatus="vs">
											<tr>
												<td class='center' ><label><input
														type='checkbox' name='ids' value="${ps.POS_ID}" /><span
												...
												<td style="width: 30px;" class="center">
														<a class="btn btn-small btn-info" title="编辑" onclick="edit('${ps.POS_ID }','${ps.UNIT_ID }');"><i class="icon-edit"></i></a>
														<a class="btn btn-small btn-danger" onclick="del('${ps.POS_ID }');" title="删除"><i class="icon-trash"></i></a>
												</td>
											</tr>
										</c:forEach>
									</c:when>
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			window.location="<%=basePath%>position/to_Add.do";
		}
		
		//修改
		function edit(POS_ID,UNIT_ID){
			window.location="<%=basePath%>position/toEdit.do?POS_ID="+POS_ID + "&UNIT_ID=" + UNIT_ID;
		}
		
		//详情
		function  info(POS_ID,UNIT_ID){
			window.location="<%=basePath%>position/toInfo.do?POS_ID="+POS_ID + "&UNIT_ID=" + UNIT_ID;
			
		}
			
		//删除
		function del(POS_ID,msg){
			bootbox.confirm("确定要删除吗?", function(result) {

				if(result) {
					
					top.jzts();
					var url = "<%=basePath%>position/deleteP.do?POS_ID="+POS_ID+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>position/deleteAllP.do?tm='
																+ new Date()
																		.getTime(),
														data : {
															IDS : str
