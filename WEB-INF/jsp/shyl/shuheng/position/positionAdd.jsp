...
<body>
<br>
	<form action="<%=basePath%>position/${msg }.do" name="Form" id="Form" method="post">
		<div id="zhongxin">
			<table class="table ">
				<tr>
				...
				<tr>
					...
					<td><select class="chzn-select" name="DEPART_ID" id="DEPART_ID"
						data-placeholder="请选择部门名称"  style="height: 20px; width: 315px;">
				...
				<tr>
					<td style="width: 80px; text-align: left; padding-top: 13px;"><B>职位名称</B></td>
					<td><input type="text" name="POS_NAME" id="POS_NAME" maxlength="40"
						 style="height: 20px; width: 300px;" placeholder="请输入职位名称"
						title="职位名称" onblur="trim(this);" /></td>
				</tr>
				<tr>
					<td nowrap="nowrap"
						style="width: 80px; text-align: left; padding-top: 13px;"><B>简介</B></td>
					<td><textarea id="INTRODUCTION" name="INTRODUCTION" maxlength="10000" 
	...
	<script type="text/javascript">
		...
		//返回
	    function returnback(){
	    	window.location.href="<%=basePath%>position/list.do";
		}
...
<!-- 附件上传 -->
	...
	//文件上传
	 $("#file_upload").fileupload({
        ...
        url: '<%=basePath%>position/upload.do',
		sequentialUploads : true,
		add: function (e, data) {
			var oldpath = $("#appendix_path").val();
			 data.url = '<%=basePath%>position/upload.do?oldpath='+oldpath;
			 data.submit();
		}
	...
	function removeAppendix(){
		...
		$.post('<%=basePath%>position/deleteAppendix.do', {path:path}, function(data){
			if(data == 'success'){
				$("#docName").val('');
				$("#appendix_path").val('');
				$("#appendix_name").val('');
