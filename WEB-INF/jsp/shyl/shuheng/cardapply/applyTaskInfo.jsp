	...
	<form action="<%=basePath%>workflow/submit_task.do" name="Form" id="Form" method="post">
		...
		<table style="width:95%;" >
			<tr>
				...
				<td style="padding-top: 20px;"><input type="text" id="apply_card_type" disabled="disabled" value="<c:if test='${pd.apply_card_type==5}'>活动证</c:if><c:if test='${pd.apply_card_type==6}'>舞厅证</c:if><c:if test='${pd.apply_card_type==7}'>停车证</c:if>" name="apply_card_type" style="width:250px;" /></td>
			</tr>
			<tr>
			...
			<c:choose>
				<c:when  test="${pd.apply_card_type==7}">
					...
					<td style="padding-top: 20px;"><img style="width:400px;" src="${img_host}${pd.apply_car_img}" title="驾驶证"/></td>
					</tr>
					<tr>
			...
			<tr>
				<td style="width: 100px; text-align: left;padding-left:20px; padding-top: 13px;"><B>批注：</B></td>
				<td style="padding-top: 20px;">
					<textarea type="text" id="remark" name="remark"  style="width:500px; height:90px;"> </textarea>
				</td>
			</tr>
			<tr>
		...
		<table id="table_report"
			...
			<thead>
				<tr>
					...
					<th class="center" style="width:20%;">批注人</th>
					...
					<c:when test="${not empty commentList}">
							<c:forEach items="${commentList}" var="list" varStatus="vs">
								<tr>
									...
									<td class="center">${list.fullMessage }</td>
								</tr>
							</c:forEach>
					</c:when>
					<c:otherwise>
