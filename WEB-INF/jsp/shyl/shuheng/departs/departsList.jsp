...
<body>
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="<%=basePath%>departs/list.do" method="post" name="Form" id="Form">
						<table style="margin-bottom: 20px;">
							<tr>
								<td class="form-inline"><span class="input-icon"> <input
										autocomplete="off" id="nav-search-input" type="text"
										name="DEPART_NAME" style="width: 350px" value="${pd.DEPART_NAME }"
										placeholder="这里输入部门名称" onblur="trim(this);" /> <i id="nav-search-icon"
										class="icon-search"></i>
								</span></td>
						...
						<table id="table_report"
							...
							<thead>
								<tr>
									...
									<th class="center" style="width:20%;">部门名称</th>
										...
										<c:forEach items="${varList}" var="ps" varStatus="vs">
											<tr>
												<td class='center' ><label><input
														type='checkbox' name='ids' value="${ps.DEPART_ID}" /><span
														class="lbl"></span></label></td>
												<td class='center' >${vs.index+1}</td>
												<td class="center">${ps.UNIT_NAME_ABBREVIATION }</td>
												<td class="tds"><a href="javascript:void(0);"
													title="${ps.DEPART_NAME }" onclick="info('${ps.DEPART_ID }');">${ps.DEPART_NAME }</a></td>												
												<td class="center">${ps.USERNAME }</td>
												<td class="center">${ps.insert_timestamp }</td>
												<td class="center">${ps.update_timestamp}</td>
												<td style="width: 30px;" class="center">
														<a class="btn btn-small btn-info" title="编辑" onclick="edit('${ps.DEPART_ID }');"><i class="icon-edit"></i></a>
														<a class="btn btn-small btn-danger" onclick="del('${ps.DEPART_ID }');" title="删除"><i class="icon-trash"></i></a>
												</td>
											</tr>
										</c:forEach>
									</c:when>
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			window.location="<%=basePath%>departs/to_Add.do";
		}
		
		//修改
		function edit(DEPART_ID){
			window.location="<%=basePath%>departs/toEdit.do?DEPART_ID="+DEPART_ID;
		}
		
		//详情
		function  info(DEPART_ID){
			window.location="<%=basePath%>departs/toInfo.do?DEPART_ID="+DEPART_ID;
		}
			
		//删除
		function del(DEPART_ID,msg){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>departs/deleteP.do?DEPART_ID="+DEPART_ID+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>departs/deleteAllP.do?tm='
																+ new Date()
																		.getTime(),
														data : {
															IDS : str
