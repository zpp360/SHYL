...
<body>
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<form action="<%=basePath%>volunteerNews/list.do" method="post" name="Form" id="Form">
					<table style="margin-bottom: 20px;">
						<tr>
							<td class="form-inline">
								<label class="lbl" for="end_time">新闻标题：</label>
								<span class="input-icon"> 
									<input autocomplete="off" id="news_title" type="text" name="news_title" value="${pd.news_title}" placeholder="这里输入新闻标题" onblur="trim(this);" /> 
					...
					<table id="table_report" class="table table-striped table-bordered table-hover">
						<thead>
							<tr>
								...
								<th class="center" style="width:30%;">新闻标题</th>
								<th class="center" style="width:20%;">来源</th>
								<th class="center" style="width:20%;">发布时间</th>
								<th class="center" style="width:12%;">更新时间</th>
								<c:if test="${!pd.superAdmin}">
								...
								<c:when test="${not empty listPage}">
										<c:forEach items="${listPage}" var="list" varStatus="vs">
											<tr>
												...
												<td class="center tds"><a href="javascript:void(0);" title="${list.news_title}" onclick="info('${list.news_id}');">${list.news_title}</a></td>	
												<td class="center tds" title="${list.news_source}">${list.news_source}</td>
												<td class="center tds" title="${list.release_time}">${list.release_time}</td>
												<td class="center tds" title="${list.update_time}">${list.update_time}</td>
												<c:if test="${!pd.superAdmin}">
													<td class="center" style="width: 50px;">
														<c:choose>
															<c:when test="${pd.isAssociation=='true'&&list.association_id!=null}">
																<a class="btn btn-mini btn-info" title="编辑" onclick="update('${list.news_id}');"><i class="icon-edit"></i></a>
															</c:when>
															<c:when test="${pd.isOrganization}">
																<a class="btn btn-mini btn-info" title="编辑" onclick="update('${list.news_id}');"><i class="icon-edit"></i></a>
															</c:when>
															<c:otherwise>
...
<%-- 														<c:if test="${pd.isAssociation=='true'&&list.association_id!=null}"> --%>
<%-- 															<a class="btn btn-mini btn-info" title="编辑" onclick="update('${list.news_id}');"><i class="icon-edit"></i></a> --%>
<%-- 														</c:if>	 --%>
<!-- 														<a class="btn btn-mini btn-info" title="编辑" style="opacity: 0.2;cursor:not-allowed;"><i class="icon-edit"></i></a> -->
														<a class="btn btn-mini btn-danger" onclick="deleteNews('${list.news_id}');" title="删除"><i class="icon-trash"></i></a>
													</td>
												</c:if>
											</tr>
										</c:forEach>
	...
	<script type="text/javascript">
		...
		//新增
		function add(){
			window.location = "<%=basePath%>volunteerNews/toAdd.do";
		}
		//详情
		function info(news_id){
			window.location = "<%=basePath%>volunteerNews/toInfo.do?news_id="+news_id;
		}
		//修改
		function update(news_id){
			window.location = "<%=basePath%>volunteerNews/toUpdate.do?news_id="+news_id;
		}
		...
		function deleteNews(news_id){
			bootbox.confirm("确定要删除吗?", function(result){
				if(result){
					...
					$.ajax({
						type : "POST",
						url : "<%=basePath%>volunteerNews/deleteNews.do?tm=" + new Date().getTime(),
						dataType : "text",
						cache : false,
		...
		function deleteBatch(){
			...
			bootbox.confirm("确定要删除选中的数据吗?", function(result){
				if(result){
					...
					$.ajax({
						type : "POST",
						url : "<%=basePath%>volunteerNews/deleteBatch.do?tm=" + new Date().getTime(),
						dataType : "json",
						cache : false,
						data :{str:str},
						success:function(data){
