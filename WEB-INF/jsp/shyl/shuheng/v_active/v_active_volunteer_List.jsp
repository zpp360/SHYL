	...
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			<div class="row-fluid">
				<div class="row-fluid">
					<!-- 检索  -->
					<form action="<%=basePath%>active/volunteerList.do" method="post" name="Form" id="Form">
						...
						<table id="table_report"
							...
							<thead>
								<tr>
									...
									<th class="center" style="width:40%;">志愿者名称</th>
									<th class="center" style="width:30%;">是否签到</th>
									<th class="center" style="width:22%;">签到时间</th>
								</tr>
							</thead>
							<tbody>
								<!-- 开始循环 -->
								<c:choose>
									<c:when test="${not empty volunteerlistPage}">
											<c:forEach items="${volunteerlistPage}" var="list" varStatus="vs">
												<tr>
													<td class='center' >
														<label>
															<input type='checkbox' name='ids' value="${list.relation_id}" />
															<span class="lbl"></span>
														</label>
													</td>
													...
													<td class="center">
														<c:if test="${list.sign_flag=='0'}">未签到</c:if>
														<c:if test="${list.sign_flag=='1'}">已签到</c:if>
													</td>
			...
			bootbox.confirm(msg, function(result) {
				if(result) {
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: "<%=basePath%>active/deleteBatchV.do",
							data : {
								str : str
							},
							dataType : "text",
