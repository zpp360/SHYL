								...
								<c:forEach items="${resPdList}" var="var" varStatus="vs">
									<tr>
										<c:choose>
											<c:when test="${fn:length(var.file_name) >30}">
												<td title="${var.file_name}"
													style="width: 140px; taxt-align: center">${fn:substring(var.file_name,0, 30)}...</td>
											</c:when>
											<c:otherwise>
												<td title="${var.file_name}"
										...
										<td style="text-align: center;"><a class="zview"
											...
											onclick="return viewPdf('${vs.index}');">预览</a><a data-toggle='modal'
											data-target='#delComAlterFile' onclick="delComAlterFile('${var.id}');" class="delete" href="javascript:void(0);">删除</a> 
										<input id="pdfPath${vs.index}" name="pdfPath${vs.index}" type="hidden" value="${var.file_path}" />
										</td>
									</tr>
								</c:forEach>
							</c:when>
