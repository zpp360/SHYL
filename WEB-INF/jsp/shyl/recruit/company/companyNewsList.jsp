...
<!DOCTYPE html>
<html lang="en">
<title>企业新闻_济南市服务外包公共服务平台</title>
<meta charset="UTF-8" />
<base href="<%=basePath%>">
	...
	<div class="register-box p0">
		...
		<div class="mypage_right">
			...
			<div class="tab" style="margin-bottom:0;">
				<a href="<%=basePath%>companyNews/create.act" class="more" style="font-size: 1em;">+&nbsp;&nbsp;新增</a>
				<form action="<%=basePath%>companyNews/list.act" method="post" name="newsform" id="newsform">
			...
			<div class="jianli">
				<table class="tableclass">
					<thead>
						<tr>
							<th style="padding-left: 30px;text-align: left;">新闻标题</th>
					...
					<tbody>
					    <c:choose>
					        <c:when test="${not empty companyNewsList}">
					            <c:forEach items="${companyNewsList}" var="var" varStatus="vs">
									<tr>
										<td class="tds" style="padding-left: 30px;text-align: left;" title="${var.title}">
										    <a href="<%=basePath%>companyNews/toNewsInfo.act?companyId=${var.company_id}&id=${var.id}" target="_blank">${var.title}</a>
										</td>
										<td style="text-align: center;">${var.insert_timestamp}</td>
										<td style="text-align: center;">
										    <a href="<%=basePath%>companyNews/toUpdateNews.act?&id=${var.id}" class="modify">修改</a>
										    <a class="delete" style="cursor: pointer;" data-toggle="modal" title="删除企业新闻" id="delnews_${var.id}" onclick="del(${var.id},'delnews_${var.id}');" data-target="#delnews">删除</a>
										</td>
									</tr>
							    </c:forEach>
							</c:when>
			...
			<div class="modal-dialog modal-sm">
				<div class="modal-content">
					...
					<div class="modal-body">
						<p id="warntext">确认删除该企业新闻吗？</p>
					</div>
					<div class="modal-footer">
						<input type="hidden" id="id" value=""> 
						<input type="hidden" id="company_id" value="${pd.company_id}">
						<input type="hidden" id="newsTitle" value="${pd.newsTitle}">	
...
<script type="text/javascript">
    function Delete() {
    	$.ajax({
			type : "POST",
			url : '<%=basePath%>companyNews/delNews.act',
			data : {
