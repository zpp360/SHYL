...
<body id="top" style="background-color: #f7f7f7;">
	...
	<div class="register-box p0">
		...
		<div class="mypage_right">
			...
			<form action="<%=basePath%>municipalUploadCommon/saveUpLoadFile.act" method="post" id="form">
			...
			<div id="files" class="col-sm-10">
				<table class="tableclass" style="margin-left: 15px">
					...
					<tbody id="fileID">
						<c:choose>
							<c:when test="${not empty resPdList}">
								<c:forEach items="${resPdList}" var="var" varStatus="vs">
									<tr>
										<c:choose>
										...
										<input id="pdfPath${var.id}" name="pdfPath${var.id}" type="hidden" value="${var.file_path}" />
										</td>
									</tr>
								</c:forEach>
							</c:when>
			...
			if(data.result!=undefined && data.result!=""){
		   		...
		   		$.ajax({
		   			...
		   			success : function(data) {
		   				reloadAjaxData(data.expensePdList,'fileID');
	...
	function delFile(){
		...
		if(id !=''){
			...
			$.ajax({
					type : "POST",
					url :'<%=basePath%>municipalUploadCommon/delFile.act',
					data : {
						id : id,
						file_type : file_type,
						info_id : $("#info_id").val()
					...
					success : function(data) {
						if (data.ret > 0) {
							...
							reloadAjaxData(data.retpd,'fileID');
						} else {
	 ...
	 function OK(url) {
		 ...
		 $("#form").attr('action','<%=basePath%>municipalUploadCommon/toNextPage.act');
		 $("#form").submit();
		 }
	 }
	    ...
	    function reloadAjaxData(List,id){
	    	...
	    	if(List ==null || List.length==0){
		    	...
		    	for (var i = 0; i < List.length; i++) {
		    		...
		    		$("#"+id).append(htmlAppend(file_name,List[i].id,List[i].file_path));
		    	}
	    	}
	    }
