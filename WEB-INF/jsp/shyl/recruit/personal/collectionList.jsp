...
<body id="top" style="background-color: #f7f7f7;">
	...
	<div class="register-box p0">
		...
		<jsp:include page="../../common/left.jsp"><jsp:param value="11" name="menuType"/></jsp:include>
		<div class="mypage_right">
			<h2>收藏夹</h2>
			<div class="tab" style="margin-bottom:0;">
				<form action="<%=basePath%>collectionInfo/list.act" method="post" name="collectionform" id="collectionform">
					<input type="text" class="search" id="nameTitle" name="nameTitle" value="${pd.nameTitle}" placeholder="请输入名称关键字" onblur="trim(this);" style="background-color: #ecf0f9;">
			...
			<div class="jianli">
				<form action="<%=basePath%>company/collectionInfo/list.act"
					...
					<table class="tableclass">
						<thead>
							<tr>
								<th style="padding-left: 30px;text-align:left;width:28%">企业名称</th>
								<th style="padding-left: 30px; text-align: left;width:25%">职位名称</th>
							...
							<c:forEach items="${varList}" var="var" varStatus="vs">
								<tr>
									<td class="tds"  style="padding-left: 30px;text-align: left;"><a
										href="<%=basePath%>company/info.act?companyId=${var.user_id}"
										target="_blank" title="${var.company_name}">${var.company_name}</a></td>
									<td class="tds"  style="padding-left: 30px;text-align: left;"><a
										id="position_id${vs.index}" href="javascript:void(0);" 
										target="_blank" title="${var.position_name}" onclick="return showJob('position_id${vs.index}','${var.user_id}','${var.recruitment_info_id}');">${var.position_name}</a></td>
									<td style="text-align: center;">${var.update_timestamp}</td>
									<td style="text-align: center;"><a href="#" class="delete"
										data-toggle="modal" id="deljob_${var.record_id}"
										onclick="del(${var.record_id});" data-target="#deljob"> <span
											class="delete"></span>取消
									</a></td>
								</tr>
							</c:forEach>
						</tbody>
	...
	<div class="modal_h20 fade bs-example-modal-sm in" id="deljob"
		...
		<div class="modal-dialog modal-sm">
			<div class="modal-content">
				...
				<div class="modal-body">
					<p id="warntext">收藏职位取消后不可恢复，确定取消吗？</p>
				</div>
	...
	<script type="text/javascript">
	    function Delete() {
	    	var url= "<%=basePath%>collectionInfo/del.act";
	        $.post(url, { id: $("#collectionInfoId").val()}, function (data) {
	        	if(data.result==1){
	        		location.href = '<%=basePath%>collectionInfo/list.act';
	        	}else{
	        		alert("删除失败");
	        	}
