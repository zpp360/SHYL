...
<body style="background-color: #f7f7f7;margin-top:-24px">
	...
	<div class="register-box p0" style="margin-top: 155px;">
		...
		<div class="mypage_right">
			<h2>简历管理</h2>
			<div class="tab">
					<a href="javascript:void(0);"  id="a2" onclick="tab0()">我的简历</a>
					<a href="javascript:void(0);"  onclick="tab1()" id="a3">谁看了我的简历</a>
					<a href="javascript:void(0);"  onclick="tab2()" id="a4">简历投递记录</a>
					<a href="javascript:void(0);" class="hover" onclick="tab3()" id="a5">我的职位邀请</a>
			</div>
			...
			<form action="<%=basePath%>myresume/myInvitationlist.act" method="post" name="Form" id="Form">
			...
			<div id="invitationlist" style="" class="jianli">
				<ul class="bg1">
					<li class="a1">企业名称</li>
					<li class="a1">职位名称</li>
					<li class="a1" style="text-align: center;">面试时间</li>
					<li class="a1">备注信息</li>
				</ul>
				<c:choose>
			   <c:when test="${not empty invitationlist}">
				<c:forEach items="${invitationlist}" var="var" varStatus="vs">
					<ul>
						<li class="a1 n-tit"><a href="<%=basePath%>company/info.act?companyId=${var.company_id}" target="_blank" title="${var.COMPANY_NAME}">${var.COMPANY_NAME}</a></li>
						<li class="a1 n-tit">
						<a id="position_id${vs.index}" href="javascript:void(0);" target="_blank" title="${var.POSITION_NAME}" onclick="return showJob('position_id${vs.index}','${var.company_id}','${var.recruitment_info_id}');">${var.POSITION_NAME}</a>
						</li>
						<li class="a1" style="text-align: center;">${var.application_time}</li>
						<li class="a1 n-tit"><a href="javascript:void(0)" onclick="invitationDetail('${var.remarks}')" title="${var.remarks}">${var.remarks}</a></li>
					</ul>
				</c:forEach>
				</c:when>
				<c:otherwise>
							...
							<input value="${fn:length(invitationlist)} " id="opendd" type="hidden">
	...
	function showJob(id, companyId, recruitmentInfoId) {
			...
			$.ajax({
				...
				success : function(data) {
					if(data.result=="0"){
						$("#" + id).attr('href','<%=basePath%>job/detail/info.act?companyId=' + companyId + '&positionId=' + recruitmentInfoId + '&flag=findJob');
						ret=true;
					}else{
...
<script type="text/javascript" src="<%=basePath%>shyl/js/myresume/myresume3.js"></script>
</body>
</html>
