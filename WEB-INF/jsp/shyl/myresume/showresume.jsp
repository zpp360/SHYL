...
<link rel="stylesheet" href="<%=basePath%>shyl/css/bootstrap_pre.css" />
...
<body>
	...
	<div class="container">
		...
		<form class="form-horizontal" action="" method="post" id="listForm" name="listForm">
			<div class="tabjl">
				<a href="javascript:void(0);" class="more" onclick="creatPDF()"
					id="creatPDF" style="position:relative;top:20px;">&nbsp;&nbsp;生成简历</a>
					<input type="hidden" id="filename" value="${downhref}">
					<input type="hidden" id="filename1" value="${filename}">
			</div>
			</form>
				...
				<form action="" novalidate="novalidate">
					...
					<div class="col-md-9 col-xs-9 col-sm-9 col-lg-9">
						<div class="form-horizontal form_txt form_view">
							...
							<div class="form-group">
								...
								<div class="col-sm-4">
									<p class="form-control-static" style="word-break:break-word;">${pd.REGISTERED_RESIDENCE }</p>
								</div>
								<label class="col-sm-2 control-label" >居住地</label>
								<div class="col-sm-4">
									<p class="form-control-static" style="word-break:break-word; ">${pd.RESIDENCE }</p>
								</div>
							</div>
							<div class="form-group">
					...
					<div class="row">
						<div class="col-md-12 col-xs-12 col-sm-12 .col-lg-12">
							<c:choose>
								<c:when
									test="${pd.user_picture !=null and pd.user_picture !=''}">
									<img
										onerror="showImgDelay(this,'<%=basePath%>shyl/images/mylogo.jpg');"
										src="<%=basePath%>images/${pd.user_picture}" alt="LOGO"
										style="max-width: 100%; max-height: 300px; margin-left: 15px;">
								</c:when>
								<c:otherwise>
									<img src="<%=basePath%>shyl/images/mylogo.jpg" alt="LOGO"
										style="max-width: 100%; max-height: 100%;">
								</c:otherwise>
							</c:choose>
						</div>
						<div class="col-md-12 col-xs-12 col-sm-12 .col-lg-12">
							<span style="color: #f00; font-size: smaller">(在我的控制台账号信息功能中可以上传头像)</span>
						</div>
						</div>
					</div>
					</div>
					...
					<div>
						<pre class="rc_qy_2">${cvpd.self_appraisal}</pre>
					</div>
					<div class="clearfix"></div>
					<div class="page-header">
						<h3>
						...
						<div class="col-sm-12">
							<c:forEach items="${ediucationpdList}" var="var" varStatus="vs">
								<p class="form-control-static">${var.timest}-
									${var.timeend}：${var.school_name }&nbsp;/&nbsp;${var.specialty }&nbsp;/&nbsp;${var.education }&nbsp;/&nbsp;${var.overseas_study}海外学习经历</p>
				...
				<div>
					...
					<div class="form-group">
						<div class="pull-right">
							<a class="btn btn-success btn-ok" href="<%=basePath%>myresume/list.act">返回</a>
						</div>
					</div>
				</div>
			</div>
	...
	function creatPDF(){
		var CV_ID=$("#cvidd").val();
		//通过Ajax向后台传值
		window.location.href='<%=basePath%>word/jl.act?CV_ID='+ CV_ID;
	}
	function down(){
		var filename=$("#usernamee").text();
		var filepath=$("#filename").val();
		var url="<%=basePath%>word/downfile.act?filepath="+encodeURI(encodeURI($("#filename").val()));
		$.ajax({
			type : "POST",
			url : url,
			dataType : 'json',
			...
			success : function(data) {
				if(data.responseStr=="OK"){
					$("#listForm").attr("action","<%=basePath%>word/download.act?fileName=" +filename+'.doc'+"&filePath=" + filepath);
					$("#listForm").submit();
				}else{
