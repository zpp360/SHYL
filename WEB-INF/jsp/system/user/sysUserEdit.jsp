...
<body>
	<form action="sysUser/editU.do" name="userForm" id="userForm" method="post">
		...
		<div id="zhongxin">
			<table class="table ">
				<tr>
					...
					<td>
						<label id="label0" class="control-label" style="padding-top: 8px; width: 100px; float: right; display: none;" for="unit"> 所属单位：</label>
					...
					<td id="td0" style="display: none;">
						<select class="chzn-select" name="unit" id="unit" data-placeholder="请选择所属单位" style="vertical-align: top;" >
							...
							<c:forEach items="${pdList}" var="list">
								<option value="${list.unit_id}" <c:if test="${list.unit_id == pd.v_unit_id }">selected</c:if>>${list.unit_name_full}</option>
							</c:forEach>
						</select>
					</td>
					<td id="td1" style="display: none;">
						<select class="chzn-select" name="association_id" id="association_id" data-placeholder="请选择志愿者协会" style="vertical-align: top;" >
							...
							<c:forEach items="${associationList}" var="list">
								<option value="${list.association_id}" <c:if test="${list.association_id == pd.association_id }">selected</c:if>>${list.association_name}</option>
							</c:forEach>
						</select>
					</td>
					<td id="td2" style="display: none;">
						<select class="chzn-select" name="organization_id" id="organization_id" data-placeholder="请选择志愿者组织" style="vertical-align: top;" >
							...
							<c:forEach items="${organizationList}" var="list">
								<option value="${list.organization_id}" <c:if test="${list.organization_id == pd.organization_id }">selected</c:if>>${list.organization_name}</option>
							</c:forEach>
						</select>
					</td>
					<td id="td3" style="display: none;">
						<select class="chzn-select" name="volunteer_unit_id" id="volunteer_unit_id" data-placeholder="请选择需求单位" style="vertical-align: top;" >
							...
							<c:forEach items="${volunteerUnitList}" var="list">
								<option value="${list.unit_id}" <c:if test="${list.unit_id == pd.volunteer_unit_id }">selected</c:if>>${list.unit_name}</option>
							</c:forEach>
						</select>
					</td>
				</tr>
				<tr>
					<td>
						<label class="control-label" style="padding-top: 8px; width: 100px; float: right;" for="loginname">用户名：</label>
					</td>
					<td>
						<input type="text" name="USERNAME" id="loginname" value="${pd.USERNAME}" maxlength="32" class="gbkOrAbc" placeholder="这里输入用户名" title="用户名" onblur="trim(this);"/>
					</td>
					<td>
						<label class="control-label" style="padding-top: 8px; width: 100px; float: right;" for="BZ">备&#12288;&#12288;注：</label>
					</td>
					<td>
						<input type="text" name="BZ" id="BZ" value="${pd.BZ}" placeholder="这里输入备注" maxlength="64" title="备注" onblur="trim(this);"/>
					</td>
<!-- 					<td> -->
...
<!-- 				</tr> -->
				<tr>
					...
					<td>
						<input type="text" name="NAME" id="name" value="${pd.NAME}" class="gbkOrName" maxlength="32" placeholder="这里输入联系人" title="联系人" onblur="trim(this);"/>
					</td>
					<td>
						<label class="control-label" style="padding-top: 8px; width: 100px; float: right;" for="PHONE">手机号：</label>
					</td>
					<td>
						<input type="text" name="PHONE" id="PHONE" value="${pd.PHONE}" maxlength="11" placeholder="这里输入手机号" title="手机号" onkeyup="value=value.replace(/[^0-9]/g,'')" onblur="delZero(this);" style="ime-mode:disabled"/>
					</td>
				</tr>
<!-- 				<tr> -->
<!-- 					<td> -->
...
<!-- 				</tr> -->
				...
				<tr>
					...
					<td>
						<input type="checkbox" name="is_super_admin"  value="1" <c:if test="${pd.IS_SUPER_ADMIN==1 }">checked</c:if> placeholder="选择是否是超级管理员" title="是否是超级管理员" style="opacity:1;position: relative;"/>
					</td>
				</tr>
				</c:if>
				<tr>
